...
scene=display(title='Angry Test', x=0, y=0, width=800, height=600)
scene.select()
scene.range=15000
fps=100
...
class Earth:
    ...
    def hit(self,o):
        if mag(self.planet.pos - o.pos) <= self.planet.radius:
            return True
    def is_in_atm(self, o):
        if mag(self.atm.pos - o.pos) <= self.atm.radius:
            return True
    def friction(self, o):
        if self.is_in_atm(o):
            #fd=norm(o.v) * 1000 * self.atm_friction
            fd=norm(o.v) * self.atm_friction* 1000 * math.pow(mag(o.v)*1000,2) / (mag(o.pos - self.atm.pos) - self.radius)
            if mag(o.v)< mag(fd):
                o.v=vector(0,0,0)
            else:
                o.v-= fd
...
class Launcher:
    ...
    def launch(self, start, vect):
        print "Launching angry ball from %s with speed %s" % (str(start),
                                                              str(vect))
